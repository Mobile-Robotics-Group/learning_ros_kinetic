...
ros::ServiceClient estop_client_clear_;
std_srvs::Trigger estop_srv_clear_;
...
void laserCallback(const sensor_msgs::LaserScan& laser_scan) {
    if (ping_index_<0)  {
        ...
        index_diff = (int) (1.56 / angle_increment_); // 90 degrees in terms of indeces
        ...
        ROS_INFO("Min angle: %f \nMax angle: %f", angle_min_, angle_max_);
   ...
   //
	...
	if(!laser_alarm_ && callSrv){
		ROS_WARN("Resetting service");
		estop_client_clear_.call(estop_srv_clear_);
		callSrv = false;
	}	
...
int main(int argc, char **argv) {
    ...
    estop_client_clear_ = nh.serviceClient<std_srvs::TriggerRequest>("clear_estop_service");
