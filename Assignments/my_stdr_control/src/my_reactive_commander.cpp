...
void indexCallback(const std_msgs::Float32& index_msg)
{
	g_index = (int)(index_msg.data);
...
int main(int argc, char **argv) {
    ...
    ros::Subscriber ind_subscriber = n.subscribe("lidar_index",1,indexCallback); 
    //some "magic numbers"
    ...
    while(ros::ok()) { // do forever
        ...
        // check the value of the index, if its negative, turn left, else turn right
        if(g_index > 0)
