...
const double DANG_TANGENT_APPROX = 0.2; // 5 * M_PI; // changed to be 45 degrees
...
// Store the averages of the readings globally for use with movement
double g_top_left_avg_x;
double g_top_left_avg_y;
...
// Gets difference of x distance from the wall of the bot
double getWallOrientationDiff()
{

    // halfway between left and straight ahead is 45 degrees left
    int index_45deg_left = g_index_90deg_left - (int)((M_PI / 8) / g_angle_increment);  // tried averaging 90 left and straight ahead
    
	double sumX_left = 0.0;
    double sumY_left = 0.0;

	double sumX_ahead = 0.0;
    double sumY_ahead = 0.0;

    // Get an average of 10 pings from around the left and the 45 degree location
    for (int i = -5; i < 5; i++) {
        sumX_left += cos(M_PI - ((g_index_90deg_left - i) * g_angle_increment)) * g_laser_scan.ranges[g_index_90deg_left - i];
        sumY_left += sin(M_PI - ((g_index_90deg_left - i) * g_angle_increment)) * g_laser_scan.ranges[g_index_90deg_left - i];
        
        sumX_ahead += cos(M_PI - ((index_45deg_left - i) * g_angle_increment)) * g_laser_scan.ranges[index_45deg_left - i];
        sumY_ahead += sin(M_PI - ((index_45deg_left - i) * g_angle_increment)) * g_laser_scan.ranges[index_45deg_left - i];
    }
    
    
    double left_avg_x = (sumX_left / 10);
    double left_avg_y = (sumY_left / 10);
	g_top_left_avg_x = (sumX_ahead / 10);
	g_top_left_avg_y = (sumY_ahead / 10);
	
	
	double ahead_left_dist = sqrt(pow(g_top_left_avg_x, 2) + pow(g_top_left_avg_y, 2)); //Distance from bot to further up on left wall		
	double left_dist = sqrt(pow(left_avg_x, 2) + pow(left_avg_y, 2)); //Distance from bot to left wall		
			
	
    //If the distLeftUp is off of the wall then dont turn the robot at all		
    if (abs(ahead_left_dist - left_dist) > WALL_FOLLOW_RADIUS || abs(ahead_left_dist) > 3 * WALL_FOLLOW_RADIUS){
        ROS_INFO("No wall within follow-radius");
        ros::spinOnce();
        return 0.0; //So that the robot doesn't turn at all since we got no real info about the wall
    }
    else {
        ROS_INFO("VALID");
        return g_top_left_avg_x - left_avg_x;
    }
}
...
int main(int argc, char** argv)
{
    ...
    while (ros::ok()) {
        ...
        int ping_error_range = 25;
        if ((g_radius_min < WALL_FOLLOW_RADIUS) && (g_index_min_dist_ping < g_index_tangent_left) && (abs(g_index_min_dist_ping - g_index_tangent_left) > ping_error_range)) {
            ...
            double dtheta = ((double)(g_index_min_dist_ping - g_index_90deg_left - ping_error_range)) * g_angle_increment;
        ...
        if ((g_radius_min < WALL_FOLLOW_RADIUS) && (g_index_min_dist_ping < g_index_tangent_left - ping_error_range)) {
			...
			// Begin process to go forward
			// Initiate a stop
            twist_cmd.linear.x = 0;
            twist_cmd.angular.z = 0.0;
            
            // publish 10 times to make sure it is sent and not lost
            for(int i = 0; i < 10; i++){
            	twist_commander.publish(twist_cmd);
            	ros::spinOnce();
                loop_timer.sleep();
            ...
            // Re-orient bot to be parallel to the wall 
            double orientDiff = getWallOrientationDiff(); // gets the x difference of left and ahead left
            
            double turn_speed = 0.1;
            int iter = 0; // to exit if stuck indefinitely
            
            // Check the difference of orientation wrt the wall
            // if there is a difference over some tolerance, fix it
             do{
                orientDiff = getWallOrientationDiff();
                ROS_INFO("Offset difference : %f", orientDiff);
				
				
				// if the difference is negative, turn it 
                if (orientDiff < 0) {
                    twist_cmd.angular.z = -1 * turn_speed;
                }
                else {
                    twist_cmd.angular.z = turn_speed;
                }
                twist_cmd.linear.x = 0;
                twist_commander.publish(twist_cmd);
                ros::spinOnce();
                loop_timer.sleep();
                iter++;
            }while (abs(orientDiff) >= 0.01 && iter < 150);
            ...
            double dist_ahead = g_top_left_avg_y - WALL_FOLLOW_RADIUS;

            //If infinite range then set to the max range of the lidar
            if (dist_ahead == std::numeric_limits<double>::infinity()) {
                dist_ahead = g_laser_scan.range_max;
            }

            double travel_time = 1; // dist_ahead / SPEED;
            ros::Rate drive_timer(1 / travel_time); // 1.5 to do slightly less than max sitance agead
            drive_timer.sleep();
